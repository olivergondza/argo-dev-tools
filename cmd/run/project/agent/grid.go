...
func NewGrid() (*Grid, error) {
	...
	for clusterName, setter := range clusters {
		go func() {
			...
			setter(clstr)
			err = clstr.CreateNs("argocd-agent")
			if err != nil {
				errorChan <- err
				return
			}
...
func (g *Grid) DeployArgos(manifests *Manifests) error {
	proc := g.Principal.KubectlProc("apply", "-k", manifests.Path("/control-plane/"))
	err := proc.Run()
	if err != nil {
		return err
	}

	repoServerHostname, err := g.getRepoServerHostname(err)
	if err != nil {
		return err
	}
	// TODO
...
func (g *Grid) getRepoServerHostname(err error) (string, error) {
	...
	if !found {
		return "", errors.New("cannot fund repo-server hostname")
	}
	var hostname string
